...
Choose the recomended selection ( Write in iso image mode ), and then Rufus will begin writing the Ubuntu installation media for you.     
...
* Root Partition (/) for the Kernal and system files, etc. 
* (optional) Home partition (/home) for day to day files and programs. This one isn't neccessary and you may just want a large combined root and home partition.
See [this](#3---order-of-partions) for how large to make each of these paritions.    
...
Windows will just overwrite the the master boot record on the drive, because the master boot record should show how to boot onto windows. 
...
will be installing a lot of applications to your computer. At the minimun however you will want to have 8-20 GB. 
...
Interestingly enough the order of the paritions that we put in might matter. 
...
and have access to the same files, altho I can't gaurentee this will work perfectly.   
...
For some reason it's possible for your windows update to corrupt, and fail everytime you attempt to update. 
